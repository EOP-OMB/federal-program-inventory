...
data_values.drop(remove_rows).reset_index(drop=True).to_csv('data_extracts/db_program_to_function_sub_function.csv', index=False, header=False)
data_values.drop(remove_rows).reset_index(drop=True).to_csv('source_files/2022-program-to-function-sub-function.csv', index=False, header=False)